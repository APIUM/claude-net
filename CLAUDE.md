...
src/
  hub/
    ...
    mirror.ts         # (planned, Phase M1) mirror-session registry + /api/mirror/* + /ws/mirror/{sid}
  plugin/
    plugin.ts         # MCP stdio server — bridges Claude Code to hub via WebSocket (single-file, served by hub)
  mirror-agent/      # (planned, Phase M1) local daemon: accepts hook POSTs, streams to hub, handles inject
  shared/
...
bin/
    ...
    claude-net-mirror-push  # (planned, Phase M1) tiny hook forwarder — stdin JSON → loopback POST
    claude-net-mirror-agent # (planned, Phase M1) entry script for the mirror-agent daemon
tests/
