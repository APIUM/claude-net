 ...
 * or unparseable file just yields the defaults. `home` defaults to the
 * real user home; overridable so tests can sandbox root resolution the
 * same way the recovery scan's `home` option does.
 */
export function loadHostConfig(home: string = os.homedir()): {
  roots: string[];
...
export function startHostChannel(opts: HostChannelOptions): HostChannelHandle {
  const { roots, allowDangerousSkip } = loadHostConfig(opts.home);
...
async function handleHostLaunch(
  ...
  if (taken.has(base)) {
    ...
    if (IDLE_SHELLS.has(paneCmd)) {
      const relaunch = `cd ${shellQuote(v.absolute)} && claude-channels${req.skip_permissions ? " --dangerously-skip-permissions" : ""}${req.continue_session && !dirWasCreated ? " --continue" : ""}`;
      await tmuxCapture([
        "send-keys",
        "-t",
        exactPane(base),
