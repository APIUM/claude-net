...
describe("Teams", () => {
  ...
  test("join creates team if new", () => {
    const members = teams.join("backend", "alice@host");
    expect(members).toEqual(["alice@host"]);
    expect(teams.teams.has("backend")).toBe(true);
  });

  test("join existing team adds member", () => {
    teams.join("backend", "alice@host");
    const members = teams.join("backend", "bob@host");
    expect(members).toEqual(["alice@host", "bob@host"]);
  });

  test("leave removes member", () => {
    teams.join("backend", "alice@host");
    teams.join("backend", "bob@host");
    const remaining = teams.leave("backend", "alice@host");
    expect(remaining).toBe(1);
    expect(teams.getMembers("backend")?.has("alice@host")).toBe(false);
    expect(teams.getMembers("backend")?.has("bob@host")).toBe(true);
  });
  ...
  test("leave from nonexistent team returns 0", () => {
    const remaining = teams.leave("nope", "alice@host");
    expect(remaining).toBe(0);
  });

  test("getTeamsForAgent returns correct teams", () => {
    teams.join("backend", "alice@host");
    teams.join("frontend", "alice@host");
    teams.join("backend", "bob@host");

    const aliceTeams = teams.getTeamsForAgent("alice@host");
  ...
  test("removeFromAllTeams cleans up correctly", () => {
    ...
    teams.removeFromAllTeams("alice@host");
  ...
  test("list returns all teams with member status", () => {
    registry.register("alice@host", mockWs());
    teams.join("backend", "alice@host");
    teams.join("backend", "bob@host"); // bob not registered = offline
    ...
    const alice = list[0]?.members.find((m) => m.name === "alice@host");
    const bob = list[0]?.members.find((m) => m.name === "bob@host");
