# Answering an AskUserQuestion popup with keystrokes

When Claude Code asks the user a question, it draws a popup in the
terminal. Answering it from the dashboard means sending the same
keystrokes a person at the keyboard would type: which keys move between
options, which one opens the notes box, which one submits. This file
records those key sequences, worked out by driving the real popup
(Claude Code **v2.1.178**).

The sequences are built by `buildAskUserQuestionKeys` in
`src/hub/dashboard/parsers.js`, posted to `/api/mirror/:sid/keys`, and
typed into the terminal by the mirror-agent.
